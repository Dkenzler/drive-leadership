...
document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.area){area=b.dataset.area;render();}if(b.hasAttribute('data-close'))b.closest('dialog').close();if(b.id==='reset'){area='Alle';$('#search').value='';$('#templates-only').checked=false;render();}});
if(!isDetail){$('#search').addEventListener('input',render);$('#templates-only').addEventListener('change',render);}
fetch('skills-data.json?v=public-skills', {cache:'no-cache'}).then(r=>{if(!r.ok)throw Error();return r.json();}).then(data=>{
 skills=data.skills;
 ...
 else{if(!areas.includes(area))area='Alle';render();if(skills.every(s=>s.available)){const a=document.createElement('a');a.className='how-link';a.href='downloads/drive-leadership-skills-alle.zip';a.download='';a.textContent='Alle 89 Skills als ZIP ↓';$('.catalogue-note').after(a);}}
