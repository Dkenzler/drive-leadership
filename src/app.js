...
autoplayReduce.addEventListener('change',()=>{autoPaused=autoplayReduce.matches;lastTick=null});
...
requestAnimationFrame(tickAccordion);
const wheel=document.querySelector('.wheel');const reduce=matchMedia('(prefers-reduced-motion: reduce)');let paused=reduce.matches,visible=true,wheelHovered=false,elapsed=0,previous=null,raf=null;const layers=[['.process',95],['.yourself',70],['.inner',45]].map(([s,p])=>[wheel.querySelector(s),p]);function frame(now){raf=null;if(paused||wheelHovered||!visible||document.hidden){previous=null;return}if(previous!==null)elapsed+=(now-previous)/1000;previous=now;for(const [layer,period]of layers)layer.setAttribute('transform',`rotate(${elapsed/period*360%360} 0 0)`);raf=requestAnimationFrame(frame)}function resume(){if(raf!==null)cancelAnimationFrame(raf);previous=null;raf=null;if(!paused&&!wheelHovered&&visible&&!document.hidden)raf=requestAnimationFrame(frame)}reduce.addEventListener('change',()=>{paused=reduce.matches;resume()});new IntersectionObserver(entries=>{visible=entries[0].isIntersecting;resume()}).observe(wheel);document.addEventListener('visibilitychange',resume);resume();
